...
const GEODESIC_DETAIL = 2; // Subdivision level for icosahedron
const RING_COUNT = 8; // Latitude rings
const RING_SEGMENTS = 48; // Points per ring
...
function GeodesicNetwork() {
  ...
  useFrame((_, delta) => {
    if (groupRef.current) {
      ...
      // Continuous base rotation (idle spin)
      rot.baseY += 0.002;
      ...
      // Target follows mouse with gentle influence
      ...
      // Apply rotations
      // Base Y rotation (continuous spin) + mouse influence on Y
      groupRef.current.rotation.y = rot.baseY + rot.currentY;

      // Mouse influence on X (vertical tilt) - gentle and smooth
      groupRef.current.rotation.x = rot.currentX;

      // No Z rotation (removes awkward roll)
    }
  });
